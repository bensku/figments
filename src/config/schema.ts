import z from 'zod';

export const AuthConfig = z.discriminatedUnion('type', [
    // Single-user mode for e.g. local development
    z.object({
        type: z.literal('none'),
        userId: z.string(),
        userName: z.string(),
    }),
    // Trusted proxy handles authentication and passes it on with HTTP headers
    z.object({
        type: z.literal('proxy'),
        userIdHeader: z.string(),
        userNameHeader: z.string(),
    }),
    // Demo authentication - demoToken and demoUserId cookies
    // Prevents literally everyone in Internet from accessing the environment
    // but doesn't really offer much security otherwise
    z.object({
        type: z.literal('demo'),
    }),
]);

export const ModelProvider = z.enum([
    'anthropic',
    'openai',
    'openaiCompatible',
    'openrouter',
    'vertex',
    'vertexAnthropic',
    'baseten',
    'bedrock',
    'bedrockAnthropic',
]);

export const ModelConfig = z.object({
    /**
     * Internal ID for referencing this model in Figments (e.g. in personas).
     */
    id: z.string(),

    /**
     * AI SDK provider to use.
     */
    provider: ModelProvider,

    /**
     * The model ID as understood by the provider.
     */
    model: z.string(),

    /**
     * Human-readable name shown in UI.
     */
    displayName: z.string(),

    /**
     * Custom base URL for the provider API.
     * Useful for proxies like LiteLLM or self-hosted endpoints.
     */
    baseUrl: z.string().optional(),

    /**
     * Environment variable name to read the API key from.
     * Defaults to ANTHROPIC_API_KEY or OPENAI_API_KEY based on provider.
     */
    apiKeyEnv: z.string().optional(),

    /**
     * Media types that this model can and should ingest natively
     * Figments will try process those not included here, not necessarily successfully.
     */
    supportedMediaTypes: z.array(z.string()),

    /**
     * Features supported by and enabled for this model.
     */
    features: z.array(z.string()),
});

export const FeatureConfig = z.object({
    /**
     * Feature id.
     */
    feature: z.string(),

    /**
     * Value for the feature configuration.
     */
    value: z.union([z.boolean(), z.number(), z.string()]),
});

export const ToolConfig = z.object({
    /**
     * Id of tool to enable.
     */
    tool: z.string(),

    /**
     * Tool options.
     */
    options: z.record(z.string(), z.unknown()),
});

export const PersonaConfig = z.object({
    key: z.string(),

    /**
     * Title of the persona to show on UI.
     */
    title: z.string(),

    /**
     * Id of model this persona should use.
     */
    model: z.string(),

    /**
     * What kind of persona this is. Users chat with presets, which themself
     * might chat with agents via tool calls.
     */
    type: z.enum(['preset', 'agent']).default('preset'),

    /**
     * System prompt of this persona (or part of it, anyway).
     */
    systemPrompt: z.string().nullish(),

    /**
     * If non-empty, append this to last user message's prompt when generating.
     * Not applied to node content.
     */
    promptSuffix: z.string().nullish(),

    /**
     * If set and true, and this persona is defined in user database, it gets
     * auto-imported to newly created spaces.
     */
    importByDefault: z.boolean().nullish(),

    /**
     * Configurations for model features that this persona enables.
     */
    features: z.array(FeatureConfig).default([]),

    /**
     * Local (Figments-provided) tools that are enabled for this persona.
     */
    tools: z.array(ToolConfig).default([]),

    /**
     * Agents that this persona can call. Agents are exposed to models as
     * tools, but require no new code to Figments unlike "normal" tools.
     */
    agents: z.array(z.string()).default([]),

    /**
     * Maximum tool calls per message generated by this persona.
     * If this is exceeded, the generation is immediately terminated. This
     * can be useful to catch models caught in loops.
     */
    maxToolCalls: z.int().default(10),

    /**
     * How this persona calls agents. 'tool' exposes them as tools, which may
     * be suboptimal if you have many agents.
     */
    agentCallMode: z.enum(['tool']).default('tool'),

    /**
     * If this persona IS an agent, its agent-specific configuration.
     */
    agentConfig: z
        .object({
            /**
             * Agent description and usage instructions.
             */
            description: z.string(),

            /**
             * Type of agent/tool loop to use.
             */
            loopType: z
                .enum(['interleavedThinking'])
                .default('interleavedThinking'),
        })
        .optional(),
});

export const Config = z.object({
    auth: AuthConfig,
    summarizer: z.object({
        model: z.string(),
    }),
    choices: z.object({
        enabled: z.boolean(),
        model: z.string().optional(),
    }),
    /**
     * Models this instance has access to.
     */
    models: z.array(ModelConfig),
    /**
     * Personas built-in into this Figments instance.
     */
    personas: z.array(PersonaConfig),
});

export type Config = z.output<typeof Config>;
export type ModelConfig = z.output<typeof ModelConfig>;
export type FeatureConfig = z.output<typeof FeatureConfig>;
export type ToolConfig = z.output<typeof ToolConfig>;
